...
def plot_max_irr_min_rel(data, col_names, fig_title, filename, score_type="lundberg"):
    ...
    ax2.set_ylabel("aSHAP scores" if score_type == "lundberg" else "eSHAP scores")  # approximate SHAP or exact SHAP
    # plt.title(fig_title)
...
def plot_irr_rel_summary(data, len_X, fig_title, filename, score_type="lundberg"):
    ...
    plt.xlabel('aSHAP scores' if score_type == "lundberg" else 'eSHAP scores')  # approximate SHAP or exact SHAP
    # plt.title(fig_title)
    plt.savefig(filename)
    plt.clf()
    plt.cla()
